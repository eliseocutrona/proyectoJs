...
export const productoSeleccionado = document.querySelector(
  "#producto-seleccion"
);
...
export const nombreProducto = document.querySelector(
  "#producto-seleccion-nombre"
);
export const precioProducto = document.querySelector(
  "#producto-seleccion-precio"
);

//Productos disponibles
export function leerProductos() {
  fetch("https://myjson.dit.upm.es/api/bins/6a6u")  
...
export function actualizarNombre() {
  objetoElegido = PRODUCTOS.find(
    (prod) => prod.link == productoSeleccionado.getAttribute("src")
  );
...
export function talleCalzado() {
  if (objetoElegido.clase == "calzado") {
    talle.innerHTML =
      '<option value="38">38</option><option value="M">39</option><option value="L">40</option><option value="XL">41</option><option value="XXL">42</option>';
  } else {
    talle.innerHTML =
      '<option value="S">S</option><option value="M">M</option><option value="L">L</option><option value="XL">XL</option><option value="XXL">XLL</option>';
  }
}
...
export function sumarCantidad() {
  cantidadCompra.textContent = CARRITO.reduce(
    (ac, pedido) => ac + parseInt(pedido.cantidad),
    0
  );
}
...
export function enviarAlCarrito() {
  ...
  let precio = PRODUCTOS.find(
    (produ) => produ.modelo == nombreProducto.textContent
  ).precio;
